package fr.putnami.pwt.sample.web.server.service;
...
import fr.putnami.pwt.sample.web.shared.domain.Address;
import fr.putnami.pwt.sample.web.shared.domain.Contact;
import fr.putnami.pwt.sample.web.shared.domain.Gender;
...
public class ContactServiceImpl implements ContactService {
	...
	public ContactServiceImpl() {
		...
		Collections.sort(groupAll.getMembers());
	}
	...
	public Contact savePerson(Contact contact) {
		...
		groupAll.getMembers().remove(contact);
		...
		groupAll.getMembers().add(contact);
		Collections.sort(groupAll.getMembers());
		return contact;
	}
	...
	private Contact createRandom() {
		...
		if (new Random().nextBoolean()) {
			contact.setName(nextValue(LAST_NAMES) + " " + nextValue(MALE_FIRST_NAMES));
			contact.setGender(Gender.MALE);
		} else {
			contact.setName(nextValue(LAST_NAMES) + " " + nextValue(FEMALE_FIRST_NAMES));
		...
		address.setStreet(nextValue(STREET_NAMES));
		address.setPostCode(nextValue(STREET_SUFFIX));
		address.setCity(nextValue(CITIES));
		contact.setAddress(address);

		contact.setEmails(Lists.<String> newArrayList());
		contact.getEmails().add(contact.getName().replace(" ", ".").toLowerCase() + "@" + nextValue(DOMAINS));
