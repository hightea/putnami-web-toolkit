...
public class InjectorViewCreator extends AbstractInjectorCreator {
	...
	protected void doCreate(TreeLogger logger, GeneratorContext context, SourceWriter srcWriter) {
		...
		for (InjectorWritterBeforePresent delegate : Iterables.filter(delegates, InjectorWritterBeforePresent.class)) {
			delegate.writeBeforePresent(srcWriter);
		}
		for (InjectorWritterPresent delegate : Iterables.filter(delegates, InjectorWritterPresent.class)) {
			delegate.writePresent(srcWriter);
		}
		for (InjectorWritterAfterPresent delegate : Iterables.filter(delegates, InjectorWritterAfterPresent.class)) {
