 ...
 */
package fr.putnami.pwt.core.serialization.ppc.rebind;
...
public class PcpSerializerCreator {
	...
	public PcpSerializerCreator(JClassType beanType) {
	...
	private void createSubModels(TreeLogger logger, GeneratorContext context) {
		for (JType jType : this.imports) {
			...
			if (PcpSerializerCreator.DISCARD_MODEL_TYPES.contains(jType.getQualifiedSourceName())) {
				continue;
			}
			if (jType instanceof JClassType) {
				PcpSerializerCreator creator = new PcpSerializerCreator((JClassType) jType);
	...
	private void generateStaticInitializer(TreeLogger logger, SourceWriter srcWriter) {
		...
		for (String propertyName : this.propertyTypes.keySet()) {
			...
			if (PcpSerializerCreator.COLLECTION_TYPES.contains(propertyType.getQualifiedSourceName())) {
