...
public class OutputList<T> extends List implements
		EditorCollection<T>,
		...
		HasOutputEditorFactory<T>
{
	...
	public <A extends EditorValue<T>> A getEditorForTraversal(int index) {
		EditorOutput output = editorProvider.getEditorForTraversal(true, index);
		if (output instanceof ListItem) {
			addListItem((ListItem) output);
		}
		else {
	...
	public void add(IsWidget child) {
		if (outputFactory == null && child instanceof OutputFactory) {
			outputFactory = (OutputFactory) child;
		}
		else {
