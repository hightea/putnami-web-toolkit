...
public class LabelFactoryVisitor extends AbstractVisitor {

	@Override
	public <A, B extends Editor> boolean visit(Context<B> context) {
		Editor editor = context.getEditor();
		if (editor instanceof HasLabelEditor) {
			HasLabelEditor hasLabelEditor = (HasLabelEditor) editor;
			if (hasLabelEditor.getLabelEditor() == null) {
				EditorLabel label = EditorFactoryManager.get().createLabel();
				hasLabelEditor.setLabelEditor(label);

				ModelDriver<?> driver = context.getDriver();
				ContextFactory.Util.get().createContext(driver, context, label);
			}
		}
