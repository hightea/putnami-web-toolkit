/*
 * Copyright 2009 Google Inc. Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with the
...
/**
 * This file is derived from original HandlerManager. It allows to "copy" an handler manager (use the same bus and resink the events)
 */

/**
 * Manager responsible for adding handlers to event sources and firing those handlers on passed in events. Primitive ancestor of {@link EventBus}, and
 * used at the core of {com.google.gwt.user.client.ui.Widget}.
 * <p>
 * While widget authors should continue to use
 * {@link com.google.gwt.user.client.ui.Widget#addDomHandler(EventHandler, com.google.gwt.event.dom.client.DomEvent.Type)} and
 * {@link com.google.gwt.user.client.ui.Widget#addHandler(EventHandler, com.google.gwt.event.shared.GwtEvent.Type)} , application developers are
 * strongly discouraged from using a HandlerManager instance as a global event dispatch mechanism.
 */
public class HandlerManager implements HasHandlers {
	...
	/**
	 * Creates a handler manager with a source to be set on all events fired via {@link #fireEvent(GwtEvent)}. Handlers will be fired in the order that
	 * they are added.
	 * 
	 * @param source
	 ...
	 * <p>
	 * Any exceptions thrown by handlers will be bundled into a {@link UmbrellaException} and then re-thrown after all handlers have completed. An
	 * exception thrown by a handler will not prevent other handlers from executing.
	 * <p>
	 * Note, any subclass should be very careful about overriding this method, as adds/removes of handlers will not be safe except within this
	 * implementation.
	 * 
	 * @param event
	 *           the event
